...
class GConv(nn.Module):
    def __init__(self, input_dim, hidden_dim, activation, num_layers):
        ...
        for i in range(num_layers):
            if i == 0:
                first_layer.add_module(f"trans_layer_{i}", TransLayer(input_dim, hidden_dim))
            else:
                first_layer.add_module(f"trans_layer_{i}", TransLayer(hidden_dim, hidden_dim))
        
        for i in range(num_layers):
            if i == 0:
                self.layers.append(nn.Sequential(first_layer, GINConvAggregation()))
    ...
    def forward(self, x, edge_index, batch):
        ...
        for conv, bn in zip(self.layers, self.batch_norms):
            if isinstance(conv, nn.Sequential):
                for trans_layer in conv[0]:
                    z = trans_layer(z, edge_index)
...
class InfoGraphGINDecN(AbstractGCLModel):
