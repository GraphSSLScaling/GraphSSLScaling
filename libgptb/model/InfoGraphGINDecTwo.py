...
class GConv(nn.Module):
    def __init__(self, input_dim, hidden_dim, activation, num_layers):
        ...
        for i in range(num_layers):
            # if num_layers != 1:
            if i == 0: #start
                self.layers.append(nn.Sequential(TransLayer(input_dim, hidden_dim),TransLayer(hidden_dim, hidden_dim),GINConvAggregation()))
                self.batch_norms.append(nn.BatchNorm1d(hidden_dim))
            # elif i == num_layers-1: #end
            ...
            # else: #internal
            #     self.layers.append(nn.Sequential(TransLayer(input_dim, hidden_dim),GINConv(nn.Sequential(nn.Linear(hidden_dim, hidden_dim), nn.ReLU(), nn.Linear(hidden_dim, hidden_dim)))))
            #     self.batch_norms.append(nn.BatchNorm1d(hidden_dim))
    ...
    def forward(self, x, edge_index, batch):
        ...
        for conv, bn in zip(self.layers, self.batch_norms):
            if isinstance(conv, nn.Sequential):
                ...
                z = conv[1](z, edge_index)
                z = conv[2](z, edge_index)
            else:
                z = conv(z, edge_index)
            z = self.activation(z)
            z = bn(z)
...
class InfoGraphGINDecTwo(AbstractGCLModel):
