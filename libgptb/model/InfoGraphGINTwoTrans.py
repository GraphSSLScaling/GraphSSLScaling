...
class GINConvFirstLayer(MessagePassing):
    def __init__(self, input_dim, out_dim):
        super(GINConvFirstLayer, self).__init__(aggr='add')
...
class GINConvAggregation(MessagePassing):
    def __init__(self):
        super(GINConvAggregation, self).__init__(aggr='add')
...
class GConv(nn.Module):
    def __init__(self, input_dim, hidden_dim, activation, num_layers):
        ...
        for i in range(num_layers):
            if i == 0:
                self.layers.append(GINConv(nn.Sequential(nn.Linear(input_dim, hidden_dim), nn.ReLU(), nn.Linear(hidden_dim, hidden_dim),nn.Linear(hidden_dim, hidden_dim), nn.ReLU(), nn.Linear(hidden_dim, hidden_dim))))
            else:
                self.layers.append(GINConvAggregation())
            self.batch_norms.append(nn.BatchNorm1d(hidden_dim))
            # if i == num_layers-1:
            #     self.layers.append(nn.Sequential(GINConv(nn.Sequential(nn.Linear(input_dim, hidden_dim), nn.ReLU(), nn.Linear(hidden_dim, hidden_dim))),nn.Sequential(nn.Linear(hidden_dim, hidden_dim), nn.ReLU(), nn.Linear(hidden_dim, hidden_dim))))
            #     self.batch_norms.append(nn.BatchNorm1d(hidden_dim))
            # else:
            #     self.layers.append(GINConvAggregation())
...
class InfoGraphGINTwoTrans(AbstractGCLModel):
    def __init__(self, config, data_feature):
        ...
        # fc1 = FC(hidden_dim=self.input_dim*(self.layers-1)+self.nhid)
        # fc2 = FC(hidden_dim=self.input_dim*(self.layers-1)+self.nhid)
